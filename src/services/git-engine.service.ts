...
export class GitEngineService {
  ...
  async migrateWithLfs(
    ...
    const lfsArgs = ['-c', 'credential.helper=', '-c', 'credential.helper=!f() { echo "username=x-access-token"; printf "password=%s\\n" "$GIT_GITHUB_TOKEN"; }; f', 'lfs', 'push', '--all', targetUrl];
    await this.spawnWithProgress(
      'git', lfsArgs,
      repoDir, onProgress, label, 1_800_000,
      this.githubToken ? { GIT_GITHUB_TOKEN: this.githubToken } : {}
    );
  }
  ...
  private async push(
    ...
    const env = this.githubToken ? { ...GIT_ENV, GIT_GITHUB_TOKEN: this.githubToken } : GIT_ENV;

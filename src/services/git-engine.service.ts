...
export class GitEngineService {
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      'git',
      ['-c', 'credential.helper=', '-c', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$GIT_TARGET_TOKEN"; }; f', 'lfs', 'push', '--all', targetUrl],
      repoDir, onProgress, label, 1_800_000,
      { GIT_TARGET_TOKEN: targetToken }
    );
  }

...
export class GitEngineService {
  ...
  async mirror(
    ...
    }).env({ ...GIT_ENV, GIT_BITBUCKET_TOKEN: sourceToken });
  ...
  async migrateWithLfs(
    ...
    const pushEnv: NodeJS.ProcessEnv = this.githubToken ? { GIT_GITHUB_TOKEN: this.githubToken } : {};
    const pushArgs = this.githubToken
      ? ['-c', 'credential.helper=', '-c', 'credential.helper=!f() { echo "username=x-access-token"; printf "password=%s\\n" "$GIT_GITHUB_TOKEN"; }; f', 'lfs', 'push', '--all', targetUrl]
      : ['lfs', 'push', '--all', targetUrl];

    await this.spawnWithProgress(
      'git', pushArgs,
      repoDir, onProgress, label, 1_800_000, pushEnv
    );
  }
  ...
  private async push(
    ...
    const gitConfig = this.githubToken ? [
      'credential.helper=',
      'credential.helper=!f() { echo "username=x-access-token"; printf "password=%s\\n" "$GIT_GITHUB_TOKEN"; }; f'
    ] : [];

    const envOverrides = this.githubToken ? { GIT_GITHUB_TOKEN: this.githubToken } : {};

...
export class GitEngineService {
  ...
  async mirror(
    ...
    const cloneGit = simpleGit({
      ...
      config: [
        'credential.helper=',
        'credential.helper=!f() { echo "username=x-bitbucket-api-token-auth"; echo "password=$SOURCE_TOKEN"; }; f'
      ],
      progress({ method, stage, progress, processed, total }: { method: string, stage: string, progress: number, processed: number, total: number }) {
        const count = total ? ` (${processed}/${total})` : '';
        onProgress?.(`[${source.name}] ${method}: ${stage} ${progress}%${count}`);
      },
    } as Partial<SimpleGitOptions>).env({ ...GIT_ENV, SOURCE_TOKEN: sourceToken });
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      'git', [
        '-c', 'credential.helper=',
        '-c', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$TARGET_TOKEN"; }; f',
        'lfs', 'push', '--all', targetUrl
      ],
      repoDir, onProgress, label, 1_800_000, { TARGET_TOKEN: targetToken }
    );
  }
  ...
  private async push(
    ...
    const pushGit = simpleGit({
      ...
      config: [
        'credential.helper=',
        'credential.helper=!f() { echo "username=x-access-token"; echo "password=$TARGET_TOKEN"; }; f'
      ],
      progress({ method, stage, progress, processed, total }: { method: string, stage: string, progress: number, processed: number, total: number }) {
        const count = total ? ` (${processed}/${total})` : '';
        onProgress?.(`[${repoName}] ${method}: ${stage} ${progress}%${count}`);
      },
    } as Partial<SimpleGitOptions>).env({ ...GIT_ENV, TARGET_TOKEN: targetToken });

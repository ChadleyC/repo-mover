...
export class GitEngineService {
  ...
  async mirror(
    ...
    const cloneGit = simpleGit({
      ...
      config: [
        'credential.helper=',
        `credential.helper=!f() { echo "username=x-bitbucket-api-token-auth"; echo "password=$GIT_TOKEN"; }; f`
      ],
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      'git', ['-c', 'credential.helper=', '-c', `credential.helper=!f() { echo "username=x-access-token"; echo "password=$GIT_TOKEN"; }; f`, 'lfs', 'push', '--all', targetUrl],
      repoDir, onProgress, label, 1_800_000,

...
export class GitEngineService {
  ...
  constructor(tempDir: string, targetToken: string) {
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      ...
      repoDir, onProgress, label, 1_800_000, { GITHUB_TOKEN: this.targetToken }
    );
  }
  ...
  private async push(
    ...
    }).env({ ...GIT_ENV, GITHUB_TOKEN: this.targetToken });

...
export class GitEngineService {
  ...
  async migrateWithLfs(
    ...
    const envOverrides: NodeJS.ProcessEnv = this.githubToken ? { GIT_GITHUB_TOKEN: this.githubToken } : {};

    // Inject credential helper for git lfs via -c arguments to avoid token exposure
    const pushArgs = [
      '-c', 'credential.helper=',
  ...
  private async push(
    ...
    if (this.githubToken) {
      ...
      config.push('credential.helper=!f() { echo "username=x-access-token"; printf "password=%s\\n" "$GIT_GITHUB_TOKEN"; }; f');
      envOverrides.GIT_GITHUB_TOKEN = this.githubToken;
    }

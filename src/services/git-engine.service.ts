...
export class GitEngineService {
  ...
  async mirror(
    ...
    // Ensure the helper is passed as a global config BEFORE the subcommand using raw()
    await cloneGit.raw([
      '-c', 'credential.helper=!f() { echo "username=x-bitbucket-api-token-auth"; echo "password=$BITBUCKET_TOKEN"; }; f',
      'clone', '--mirror', '--progress', source.cloneUrl, repoDir
    ]);
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      'git', ['-c', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$GITHUB_TOKEN"; }; f', 'lfs', 'push', '--all', targetUrl],
      repoDir, onProgress, label, 1_800_000,
      { GITHUB_TOKEN: targetToken }
    );
  }
  ...
  private async push(
    ...
    }).env({ ...GIT_ENV, GITHUB_TOKEN: targetToken });

    // Ensure the helper is passed as a global config BEFORE the subcommand using raw()
    await pushGit.raw([
      '-c', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$GITHUB_TOKEN"; }; f',
      'push', '--mirror', '--progress', targetUrl
    ]);
  }

...
export class GitEngineService {
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      ...
      repoDir, onProgress, label, 1_800_000,
      this.githubToken ? { GIT_GITHUB_TOKEN: this.githubToken } : undefined
    );
  }
  ...
  private async push(
    ...
    const configOpts = this.githubToken ? {
      config: [
        'credential.helper=',
    ...
    const envOpts = this.githubToken ? { GIT_GITHUB_TOKEN: this.githubToken } : {};

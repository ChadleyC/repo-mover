...
export class GitEngineService {
  ...
  async migrateWithLfs(
    ...
    const lfsPushArgs = ['lfs', 'push', '--all', targetUrl];
    let envOverrides: NodeJS.ProcessEnv | undefined;

    if (this.githubToken) {
      // Inject credentials via -c arguments because LFS might not respect simple-git's config
      lfsPushArgs.unshift(
        '-c', 'credential.helper=',
        '-c', 'credential.helper=!f() { echo "username=x-access-token"; printf "password=%s\\n" "$GIT_TOKEN"; }; f'
      );
      envOverrides = { GIT_TOKEN: this.githubToken };
    }
  ...
  private async push(
    ...
    }).env({ ...GIT_ENV, ...(this.githubToken ? { GIT_TOKEN: this.githubToken } : {}) });

...
export class GitEngineService {
  ...
  async mirror(
    ...
    const cloneGit = simpleGit({
      ...BASE_GIT_OPTIONS,
      config: ['credential.helper=', 'credential.helper=!f() { echo "username=x-bitbucket-api-token-auth"; echo "password=$BITBUCKET_TOKEN"; }; f'],
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      'git',
      ['-c', 'credential.helper=', '-c', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$GITHUB_TOKEN"; }; f', 'lfs', 'push', '--all', targetUrl],
  ...
  private async push(
    ...
    const pushGit = simpleGit({
      ...BASE_GIT_OPTIONS,
      baseDir: repoDir,
      config: ['credential.helper=', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$GITHUB_TOKEN"; }; f'],
      progress({ method, stage, progress, processed, total }) {
        const count = total ? ` (${processed}/${total})` : '';

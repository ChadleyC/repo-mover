...
export class GitEngineService {
  ...
  async mirror(
    ...
    const cloneGit = simpleGit({
      ...BASE_GIT_OPTIONS,
      config: ['credential.helper=', 'credential.helper=!f() { echo "username=x-bitbucket-api-token-auth"; echo "password=$GIT_TOKEN"; }; f'],
  ...
  private async push(
    ...
    const pushGit = simpleGit({
      ...BASE_GIT_OPTIONS,
      baseDir: repoDir,
      config: ['credential.helper=', 'credential.helper=!f() { echo "username=x-access-token"; echo "password=$GIT_TOKEN"; }; f'],
      progress({ method, stage, progress, processed, total }) {
        const count = total ? ` (${processed}/${total})` : '';
        onProgress?.(`[${repoName}] ${method}: ${stage} ${progress}%${count}`);
      },
    }).env({ ...GIT_ENV, GIT_TOKEN: targetToken });

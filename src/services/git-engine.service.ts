...
export class GitEngineService {
  ...
  async mirror(
    ...
    }).env({ ...process.env, ...GIT_ENV, BITBUCKET_TOKEN: sourceToken });
  ...
  async migrateWithLfs(
    ...
    await this.spawnWithProgress(
      'git',
      ['-c', `credential.helper=!f() { echo "username=x-access-token"; echo "password=$TARGET_TOKEN"; }; f`, 'lfs', 'push', '--all', targetUrl],
      repoDir, { TARGET_TOKEN: targetToken }, onProgress, label, 1_800_000,
    );
  }
  ...
  private async push(
    ...
    }).env({ ...process.env, ...GIT_ENV, TARGET_TOKEN: targetToken });

...
export class GitEngineService {
  ...
  async migrateWithLfs(
    ...
    let lfsPushArgs = ['lfs', 'push', '--all', targetUrl];
    let lfsPushEnv: NodeJS.ProcessEnv | undefined = undefined;
    ...
    if (this.githubToken) {
      ...
      lfsPushEnv = { GITHUB_TOKEN: this.githubToken };
    }

    await this.spawnWithProgress(
      'git', lfsPushArgs,
      repoDir, onProgress, label, 1_800_000, lfsPushEnv
    );
  }
  ...
  private async push(
    ...
    const env = this.githubToken ? { ...GIT_ENV, GITHUB_TOKEN: this.githubToken } : GIT_ENV;

    ...
    try {
      ...
      for (let i = 0; i < repositoriesToMigrate.length; i++) {
        ...
        try {
          ...
          // Construct target URL for GitHub
          ...
          try {
            await gitEngine.mirror(repo, targetUrl, config.bitbucket.token, onProgress, targetToken);
          } catch (err) {
            if (err instanceof LargeFilesError) {
              ...
              if (action === 'lfs') {
                ...
                await gitEngine.migrateWithLfs(repo, targetUrl, onProgress, targetToken);
              } else if (action === 'skip') {
                ...
                await gitEngine.stripAndPush(repo, targetUrl, onProgress, selectedFiles, targetToken);
              } else {
                reporter.info(`Stripping all large files from ${repo.name} history…`);
                await gitEngine.stripAndPush(repo, targetUrl, onProgress, undefined, targetToken);
              }
            } else {
              throw err;
            }

    ...
    try {
      ...
      for (let i = 0; i < repositoriesToMigrate.length; i++) {
        ...
        try {
          ...
          // Construct target URL for GitHub without authentication embedded securely
          ...
          try {
            ...
            if (err instanceof LargeFilesError) {
              ...
              if (action === 'lfs') {
                ...
                await gitEngine.migrateWithLfs(repo, targetUrl, config.github.token, config.bitbucket.token, onProgress);
              } else if (action === 'skip') {

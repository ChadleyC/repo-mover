    ...
    try {
      ...
      for (let i = 0; i < repositoriesToMigrate.length; i++) {
        ...
        try {
          ...
          // Construct target URL for GitHub without authentication (credentials passed securely via environment)
          const targetUrl = `https://github.com/${targetOwner}/${githubOptions.name}.git`;
          const targetToken = config.github.token;
          const onProgress = (msg: string) => reporter.log(chalk.dim(`  ${msg}`));
          
          // Mirror the repository — detects large files before pushing
          try {
            await gitEngine.mirror(repo, targetUrl, config.bitbucket.token, targetToken, onProgress);
          } catch (err) {
            if (err instanceof LargeFilesError) {
              ...
              if (action === 'lfs') {
                ...
                await gitEngine.migrateWithLfs(repo, targetUrl, targetToken, onProgress);
              } else if (action === 'skip') {
                ...
                await gitEngine.stripAndPush(repo, targetUrl, targetToken, onProgress, selectedFiles);
              } else {
                reporter.info(`Stripping all large files from ${repo.name} history…`);
                await gitEngine.stripAndPush(repo, targetUrl, targetToken, onProgress);
              }
            } else {
              throw err;
            }

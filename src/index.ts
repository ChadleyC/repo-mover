    ...
    try {
      ...
      for (let i = 0; i < repositoriesToMigrate.length; i++) {
        ...
        try {
          ...
          // Construct target URL for GitHub without embedded authentication to prevent credential exposure
          ...
          try {
            await gitEngine.mirror(repo, targetUrl, config.bitbucket.token, onProgress, config.github.token);
          } catch (err) {
            if (err instanceof LargeFilesError) {
              ...
              if (action === 'lfs') {
                ...
                await gitEngine.migrateWithLfs(repo, targetUrl, onProgress, config.github.token);
              } else if (action === 'skip') {
                ...
                await gitEngine.stripAndPush(repo, targetUrl, onProgress, selectedFiles, config.github.token);
              } else {
                reporter.info(`Stripping all large files from ${repo.name} history…`);
                await gitEngine.stripAndPush(repo, targetUrl, onProgress, undefined, config.github.token);
              }
            } else {
              throw err;
            }
